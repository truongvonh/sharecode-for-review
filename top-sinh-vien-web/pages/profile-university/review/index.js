...
const ReviewSchool = ({ university_id, toggleDownload }) => {
  ...
  const getReviewOfSchool = async ({ id, page, rating, onScrollLoad = false, limit = initPagination.limit }) => {
    try {
      const result = await SCHOOL_ENDPOINT.REVIEW_OF_SCHOOL({ id, page, limit, rating });
  ...
  return (
    <>
      <Head ogImage="/static/img/img_logo.png" title="Review trường học" />
      <div className="wrapper-review">
        <div className="w-100 d-flex justify-content-center align-items-center px-5 flex-column">
          <button
            className="mt-4 btn-review py-2 font-weight-bold fz-12 mb-4 w-100"
            onClick={() => toggleDownload()}
          >{`VIẾT REVIEW VỀ TRƯỜNG`}</button>
        </div>
        <div className="d-flex justify-content-between pb-3 flex-column flex-md-row align-items-center">
          <p className="font-weight-bold fz-18 m-0">{`${REVIEW_SCHOOL_TEXT.REVIEW_SCHOOL} (${total})`}</p>
