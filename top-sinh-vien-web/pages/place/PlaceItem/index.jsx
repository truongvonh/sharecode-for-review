...
import './../../result/component/location/location.scss';
import { ALL_TYPE } from 'constants/common';

const UserPlace = ({ avatar, name, address, rating, school, score, index, id, }) => {
    ...
    <div className="col-md-6 col-xl-3 mb-3 px-2 mx-md-0 " >
      <div className="wrapper-item-card-place d-flex p-3 shadow" >
        <div className=" place-image  flex-shrink-0 ">
          <Link href={NAVIGATE_URL.LOCATION_DETAIL_PAGE.URL(getNestedObjectSafe(id, []), ALL_TYPE.LOCATION)}>
            <a>
              <ImageWithFallback
        ...
        <div className="place-item col-8 ">
          ...
          <div className="place-rank-star overviews">
            <SvgIcons fileName="ic_overview"
              ...
              noHover
              height={10} />
            <p className="graduate-result fz-12 font-weight-bold text-truncate">{school.length ? school.map(item => item && item.schoolCode).join('/') : ''}</p>
          </div>
        </div>
      </div>
    </div>
